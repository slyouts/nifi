...
public class StatelessProcessSession implements ProcessSession {
    ...
    private final Map<Relationship, Queue<FlowFileRecord>> outputMap = new HashMap<>();
    private final Queue<FlowFileRecord> inputQueue;
    private final Set<Long> beingProcessed = new HashSet<>();
    private final List<FlowFileRecord> penalized = new ArrayList<>();
    private final Processor processor;

    private final Map<Long, FlowFileRecord> currentVersions = new HashMap<>();
    private final Map<Long, FlowFileRecord> originalVersions = new HashMap<>();
    ...
    public StatelessProcessSession(final Queue<FlowFileRecord> input, final Collection<ProvenanceEventRecord> events, final Processor processor, final Set<Relationship> outputRelationships,
            final Runnable nextStep) {
    ...
    @Override
    public FlowFileRecord putAllAttributes(FlowFile flowFile, final Map<String, String> attrs) {
        flowFile = validateState(flowFile);
        if (attrs == null || flowFile == null) {
            throw new IllegalArgumentException("argument cannot be null");
        }
        final TransactionalFlowFile newFlowFile = new TransactionalFlowFile((TransactionalFlowFile) flowFile, this.materializeContent);
    ...
    @Override
    public FlowFileRecord putAttribute(FlowFile flowFile, final String attrName, final String attrValue) {
        flowFile = validateState(flowFile);
    ...
    @Override
    public FlowFileRecord removeAllAttributes(FlowFile flowFile, final Set<String> attrNames) {
        flowFile = validateState(flowFile);
        if (attrNames == null || flowFile == null) {
            throw new IllegalArgumentException("argument cannot be null");
        }
        if (!(flowFile instanceof TransactionalFlowFile)) {
    ...
    @Override
    public FlowFileRecord removeAllAttributes(FlowFile flowFile, final Pattern keyPattern) {
        flowFile = validateState(flowFile);
        if (flowFile == null) {
            throw new IllegalArgumentException("flowFile cannot be null");
        }
        if (keyPattern == null) {
            return (TransactionalFlowFile) flowFile;
        }
    ...
    @Override
    public FlowFileRecord removeAttribute(FlowFile flowFile, final String attrName) {
        flowFile = validateState(flowFile);
    ...
    /**
     * Inherits the attributes from the given source flow file into another flow file. The UUID of the source becomes the parent UUID of this flow file. If a parent uuid had previously been
     * established it will be replaced by the uuid of the given source
     *
     * @param source
     *            the FlowFile from which to copy attributes
     * @param destination
    ...
    /**
     * Inherits the attributes from the given source flow files into the destination flow file. The UUIDs of the sources becomes the parent UUIDs of the destination flow file. Only attributes which is
     * common to all source items is copied into this flow files attributes. Any previously established parent UUIDs will be replaced by the UUIDs of the sources. It will capture the uuid of a certain
     * number of source objects and may not capture all of them. How many it will capture is unspecified.
     *
     * @param sources
    ...
    /**
     * Returns the attributes that are common to every flow file given. The key and value must match exactly.
     *
     * @param flowFileList
    ...
    private static Map<String, String> intersectAttributes(final Collection<FlowFile> flowFileList) {
        ...
        /*
         * Start with the first attribute map and only put an entry to the resultant map if it is common to every map.
         */
    ...
    public void migrate(final ProcessSession newOwner, final Collection<FlowFile> flowFiles) {
        Collection<TransactionalFlowFile> statelessFlowFiles = (Collection<TransactionalFlowFile>) (Collection) flowFiles;
        ...
        for (final TransactionalFlowFile flowFile : statelessFlowFiles) {

            final TransactionalFlowFile currentVersion = currentVersions.get(flowFile.getId());
            if (currentVersion == null) {
                throw new FlowFileHandlingException(flowFile + " is not known in this session");
            }
        }

        for (final Map.Entry<Relationship, Queue<TransactionalFlowFile>> entry : outputMap.entrySet()) {
            final Relationship relationship = entry.getKey();
            final Queue<TransactionalFlowFile> transferredFlowFiles = entry.getValue();
    ...
    public void remove(FlowFile flowFile) {
        ...
        final Iterator<TransactionalFlowFile> penalizedItr = penalized.iterator();
        while (penalizedItr.hasNext()) {
            final TransactionalFlowFile ff = penalizedItr.next();
    ...
    public void rollback(final boolean penalize) {
        ...
        for (final Queue<TransactionalFlowFile> list : outputMap.values()) {
            for (final TransactionalFlowFile flowFile : list) {
        ...
        for (final Long flowFileId : beingProcessed) {
            final TransactionalFlowFile flowFile = originalVersions.get(flowFileId);
            if (flowFile != null) {
                inputQueue.offer(flowFile);
                if (penalize) {
                    penalized.add(flowFile);
    ...
    public void transfer(FlowFile flowFile) {
        ...
        inputQueue.add(flowFile);
        updateLastQueuedDate((TransactionalFlowFile) flowFile);
    ...
    public void transfer(FlowFile flowFile, final Relationship relationship) {
        ...
        if (outputMap.containsKey(relationship)) {
            Queue<TransactionalFlowFile> queue = this.outputMap.get(relationship);
            queue.add((TransactionalFlowFile) flowFile);
    ...
    @Override
    public TransactionalFlowFile penalize(FlowFile flowFile) {
        flowFile = validateState(flowFile);
        final TransactionalFlowFile newFlowFile = new TransactionalFlowFile((TransactionalFlowFile) flowFile, this.materializeContent);
        currentVersions.put(newFlowFile.getId(), newFlowFile);
    ...
    @Override
    public TransactionalFlowFile create() {
        final TransactionalFlowFile flowFile = new TransactionalFlowFile(this.materializeContent);
    ...
    @Override
    public TransactionalFlowFile create(final FlowFile flowFile) {
        TransactionalFlowFile newFlowFile = create();
        newFlowFile = (TransactionalFlowFile) inheritAttributes(flowFile, newFlowFile);
        currentVersions.put(newFlowFile.getId(), newFlowFile);
    ...
    @Override
    public TransactionalFlowFile create(final Collection<FlowFile> flowFiles) {
        TransactionalFlowFile newFlowFile = create();
        newFlowFile = (TransactionalFlowFile) inheritAttributes(flowFiles, newFlowFile);
        currentVersions.put(newFlowFile.getId(), newFlowFile);
        beingProcessed.add(newFlowFile.getId());
        return newFlowFile;
    }
    ...
    @Override
    public TransactionalFlowFile get() {
        final TransactionalFlowFile flowFile = inputQueue.poll();
        if (flowFile != null) {
            beingProcessed.add(flowFile.getId());
            currentVersions.put(flowFile.getId(), flowFile);
    ...
    public List<FlowFile> get(final int maxResults) {
        ...
        for (int i = 0; i < maxResults; i++) {
            final TransactionalFlowFile nextFlowFile = get();
    ...
    public List<FlowFile> get(final FlowFileFilter filter) {
        ...
        final List<TransactionalFlowFile> unselected = new ArrayList<>();
    ...
    public QueueSize getQueueSize() {
        ...
        long contentSize = 0L;
        for (final TransactionalFlowFile flowFile : inputQueue) {
    ...
    // region Content
    @Override
    public TransactionalFlowFile clone(FlowFile flowFile) {
        flowFile = validateState(flowFile);
        final TransactionalFlowFile newFlowFile = new TransactionalFlowFile((TransactionalFlowFile) flowFile, this.materializeContent);
        currentVersions.put(newFlowFile.getId(), newFlowFile);
        beingProcessed.add(newFlowFile.getId());
    ...
    @Override
    public TransactionalFlowFile clone(FlowFile flowFile, final long offset, final long size) {
        flowFile = validateState(flowFile);
        try {
            ((TransactionalFlowFile) flowFile).materializeData();
        } catch (IOException e) {
        ...
        final TransactionalFlowFile newFlowFile = new TransactionalFlowFile((TransactionalFlowFile) flowFile, offset, size, this.materializeContent);
    ...
    public void exportTo(FlowFile flowFile, final OutputStream out) {
        ...
        try {
            copyTo(((TransactionalFlowFile) flowFile).getDataStream(), out);
    ...
    public void exportTo(FlowFile flowFile, final Path path, final boolean append) {
        ...
        TransactionalFlowFile statelessFlowFile = (TransactionalFlowFile) flowFile;
    ...
    @Override
    public TransactionalFlowFile importFrom(final InputStream in, FlowFile flowFile) {
        flowFile = validateState(flowFile);
    ...
    @Override
    public TransactionalFlowFile importFrom(final Path path, final boolean keepSourceFile, FlowFile flowFile) {
        flowFile = validateState(flowFile);
        ...
        TransactionalFlowFile newFlowFile = new TransactionalFlowFile((TransactionalFlowFile) flowFile, this.materializeContent);
        try {
    ...
    public void read(FlowFile flowFile, boolean allowSessionStreamManagement, final InputStreamCallback callback) {
        ...
        try {
            ((TransactionalFlowFile) flowFile).materializeData();
            callback.process(((TransactionalFlowFile) flowFile).getDataStream());
        } catch (final IOException e) {
    ...
    public InputStream read(FlowFile flowFile) {
        ...
        return ((TransactionalFlowFile) flowFile).getDataStream();
    }

    @Override
    public TransactionalFlowFile write(FlowFile flowFile, final OutputStreamCallback callback) {
        flowFile = validateState(flowFile);
        if (callback == null) {
    ...
    public OutputStream write(FlowFile flowFile) {
        ...
        final ByteArrayOutputStream baos = new ByteArrayOutputStream() {
            ...
            public void close() throws IOException {
                super.close();
                final TransactionalFlowFile newFlowFile = new TransactionalFlowFile((TransactionalFlowFile) flowFile, materializeContent);
    ...
    public FlowFile append(FlowFile flowFile, final OutputStreamCallback callback) {
        ...
        final TransactionalFlowFile validatedFlowFile = validateState(flowFile);
        ...
        final TransactionalFlowFile newFlowFile = new TransactionalFlowFile(validatedFlowFile, this.materializeContent);
        currentVersions.put(newFlowFile.getId(), newFlowFile);
    ...
    @Override
    public TransactionalFlowFile write(final FlowFile flowFile, final StreamCallback callback) {
        if (callback == null || flowFile == null) {
            throw new IllegalArgumentException("argument cannot be null");
        }
        final TransactionalFlowFile statelessFlowFile = validateState(flowFile);
        ...
        final TransactionalFlowFile newFlowFile = new TransactionalFlowFile(statelessFlowFile, this.materializeContent);
        currentVersions.put(newFlowFile.getId(), newFlowFile);
    ...
    @Override
    public TransactionalFlowFile merge(Collection<FlowFile> sources, FlowFile destination) {
        sources = validateState(sources);
        destination = validateState(destination);
        final TransactionalFlowFile newFlowFile = new TransactionalFlowFile((TransactionalFlowFile) destination, this.materializeContent);
        for (final FlowFile flowFile : sources) {
            newFlowFile.addData(((TransactionalFlowFile) flowFile).getDataStream());
        }
        currentVersions.put(newFlowFile.getId(), newFlowFile);
    ...
    @Override
    public TransactionalFlowFile merge(Collection<FlowFile> sources, FlowFile destination, byte[] header, byte[] footer, byte[] demarcator) {
        Collection<TransactionalFlowFile> statelessSources = (Collection) validateState(sources);
        TransactionalFlowFile statelessDestination = validateState(destination);
        ...
        int count = 0;
        for (final TransactionalFlowFile flowFile : statelessSources) {
        ...
        final TransactionalFlowFile newFlowFile = new TransactionalFlowFile(statelessDestination, this.materializeContent);
        currentVersions.put(newFlowFile.getId(), newFlowFile);

        return newFlowFile;
    }

    public TransactionalFlowFile unpenalize(FlowFile flowFile) {
        flowFile = validateState(flowFile);
        final TransactionalFlowFile newFlowFile = new TransactionalFlowFile((TransactionalFlowFile) flowFile, this.materializeContent);
        currentVersions.put(newFlowFile.getId(), newFlowFile);
    ...
    private FlowFileRecord validateState(final FlowFile flowFile) {
        Objects.requireNonNull(flowFile);

        final FlowFileRecord currentVersion = currentVersions.get(flowFile.getId());
        if (currentVersion == null) {
            throw new FlowFileHandlingException(flowFile + " is not known in this session");
        }

        for (final Queue<FlowFileRecord> flowFiles : outputMap.values()) {
    ...
    public Queue<FlowFileRecord> getAndRemoveFlowFilesForRelationship(final String relationship) {
    ...
    public Queue<FlowFileRecord> getAndRemoveFlowFilesForRelationship(final Relationship relationship) {
        Queue<FlowFileRecord> queue = this.outputMap.get(relationship);
        if (queue == null) {
    ...
    public List<FlowFileRecord> getPenalizedFlowFiles() {
        return penalized;
    }

    private void updateLastQueuedDate(FlowFileRecord StatelessFlowFile) {
