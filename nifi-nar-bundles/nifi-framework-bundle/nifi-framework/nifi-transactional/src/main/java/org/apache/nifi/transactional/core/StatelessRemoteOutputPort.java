...
import org.apache.nifi.transactional.bootstrap.InMemoryFlowFile;
...
public class StatelessRemoteOutputPort extends AbstractTransactionalComponent {
    ...
    public boolean runRecursive(final Queue<InMemoryFlowFile> queue) {
        try {
            ...
            final Queue<TransactionalFlowFile> destinationQueue = new LinkedList<>();
            ...
            while ((dataPacket = transaction.receive()) != null) {
                ...
                final TransactionalFlowFile receivedFlowFile = new TransactionalFlowFile(buffer, attributes, true);
                destinationQueue.add(receivedFlowFile);

                for (final TransactionalComponent childComponent : getChildren().get(Relationship.ANONYMOUS)) {
