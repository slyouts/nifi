 ...
 */
public final class TransactionalProcessScheduler implements ProcessScheduler {

    private static final Logger LOG = LoggerFactory.getLogger(TransactionalProcessScheduler.class);
    ...
    public TransactionalProcessScheduler(final FlowEngine componentLifecycleThreadPool, final StringEncryptor encryptor,
            final StateManagerProvider stateManagerProvider, final ExtensionManager extensionManager, final NiFiProperties nifiProperties) {
    ...
    public void setControllerServiceProvider(TransactionalControllerServiceProvider controllerServiceProvider) {
    ...
    public void scheduleFrameworkTask(final Runnable command, final String taskName, final long initialDelay, final long delay,
    ...
    public void schedule(final ReportingTaskNode taskNode) {
        ...
        if (activeThreadCount > 0) {
            throw new IllegalStateException(
                    "Reporting Task " + taskNode.getName() + " cannot be started because it has " + activeThreadCount + " threads still running");
        }
                ...
                try {
                    synchronized (lifecycleState) {
                        ...
                        if (!lifecycleState.isScheduled() || lifecycleState.getLastStopTime() != lastStopTime) {
                            LOG.debug(
                                    "Did not complete invocation of @OnScheduled task for {} but Lifecycle State is no longer scheduled. Will not attempt to invoke task anymore",
                                    reportingTask);
                            return;
                        }
                        ...
                        try (final NarCloseable x = NarCloseable.withComponentNarLoader(extensionManager, reportingTask.getClass(),
                    ...
                    LOG.error(
                            "Failed to invoke the On-Scheduled Lifecycle methods of {} due to {}; administratively yielding this "
            ...
            public void run() {
                ...
                synchronized (lifecycleState) {
                    ...
                    try (final NarCloseable x = NarCloseable.withComponentNarLoader(extensionManager, reportingTask.getClass(),
                        ...
                        LOG.error(
                                "Failed to invoke the @OnUnscheduled methods of {} due to {}; administratively yielding this ReportingTask and will attempt to schedule it again after {}",
    ...
    /**
     * Starts the given {@link Processor} by invoking its {@link ProcessorNode#start(ScheduledExecutorService, long, long, Supplier, SchedulingAgentCallback, boolean)} method.
     *
    ...
    public synchronized CompletableFuture<Void> startProcessor(final ProcessorNode procNode, final boolean failIfStopping) {
        ...
        final Supplier<ProcessContext> processContextFactory = () -> new StandardProcessContext(procNode, controllerServiceProvider,
        ...
        procNode.start(componentMonitoringThreadPool, administrativeYieldMillis, processorStartTimeoutMillis, processContextFactory, callback,
                failIfStopping);
        return future;
    }

    /**
     * Stops the given {@link Processor} by invoking its {@link ProcessorNode#stop(ProcessScheduler, ScheduledExecutorService, ProcessContext, SchedulingAgent, LifecycleState)} method.
     *
    ...
    public synchronized CompletableFuture<Void> stopProcessor(final ProcessorNode procNode) {
        ...
        StandardProcessContext processContext = new StandardProcessContext(procNode, controllerServiceProvider, this.encryptor,
                getStateManager(procNode.getIdentifier()), lifecycleState::isTerminated);
    ...
    private synchronized void stopConnectable(final Connectable connectable) {
        ...
        if (!state.isScheduled() && state.getActiveThreadCount() == 0 && state.mustCallOnStoppedMethods()) {
            ...
            try (final NarCloseable x = NarCloseable.withComponentNarLoader(extensionManager, connectable.getClass(), connectable.getIdentifier())) {
    ...
    /**
     * Returns the LifecycleState that is registered for the given component; if no LifecycleState current is registered, one is created and registered atomically, and then that value is returned.
     *
     * @param schedulable
     *            schedulable
