...
public class TransactionalReloadComponent implements ReloadComponent {
    private static final Logger logger = LoggerFactory.getLogger(TransactionalReloadComponent.class);
    private final ExtensionManager extensionManager;
    private final VariableRegistry varRegistry;
    ...
    public TransactionalReloadComponent(final ExtensionManager extensionManager, final VariableRegistry variableRegistry,
            final ControllerServiceProvider controllerServiceProvider, final StateManagerProvider stateManagerProvider,
            final ValidationTrigger validationTrigger, final StringEncryptor encryptor) {
        this.extensionManager = extensionManager;
        this.varRegistry = variableRegistry;
    ...
    public void reload(final ProcessorNode existingNode, final String newType, final BundleCoordinate bundleCoordinate, final Set<URL> additionalUrls)
        ...
        final ProcessorNode newNode = flowManager.createProcessor(newType, id, bundleCoordinate, additionalUrls, true, false);

        // call OnRemoved for the existing processor using the previous instance class loader
        try (final NarCloseable x = NarCloseable.withComponentNarLoader(existingInstanceClassLoader)) {
            final StateManager stateManager = stateManagerProvider.getStateManager(id);
            final StandardProcessContext processContext = new StandardProcessContext(existingNode, serviceProvider,
                    encryptor, stateManager, () -> false);
        ...
        final LoggableComponent<Processor> newProcessor = new LoggableComponent<>(newNode.getProcessor(), newNode.getBundleCoordinate(),
        ...
        validationTrigger.trigger(existingNode);
    }

    @Override
    public void reload(final ControllerServiceNode existingNode, final String newType, final BundleCoordinate bundleCoordinate,
            final Set<URL> additionalUrls) throws ControllerServiceInstantiationException {
        if (existingNode == null) {
        ...
        // attempt the creation to make sure it works before firing the OnRemoved methods below
        final ControllerServiceNode newNode = flowManager.createControllerService(newType, id, bundleCoordinate, additionalUrls, true, false);

        // call OnRemoved for the existing service using the previous instance class loader
        try (final NarCloseable x = NarCloseable.withComponentNarLoader(existingInstanceClassLoader)) {
            final ConfigurationContext configurationContext = new StandardConfigurationContext(existingNode, serviceProvider, null, varRegistry);
    ...
    @Override
    public void reload(final ReportingTaskNode existingNode, final String newType, final BundleCoordinate bundleCoordinate,
            final Set<URL> additionalUrls) throws ReportingTaskInstantiationException {
        if (existingNode == null) {
        ...
        // attempt the creation to make sure it works before firing the OnRemoved methods below
        final ReportingTaskNode newNode = flowManager.createReportingTask(newType, id, bundleCoordinate, additionalUrls, true,
                false);

        // call OnRemoved for the existing reporting task using the previous instance class loader
        try (final NarCloseable x = NarCloseable.withComponentNarLoader(existingInstanceClassLoader)) {
            ReflectionUtils.quietlyInvokeMethodsWithAnnotation(OnRemoved.class, existingNode.getReportingTask(),
                    existingNode.getConfigurationContext());
        } finally {
            extensionManager.closeURLClassLoader(id, existingInstanceClassLoader);
        }
        ...
        final LoggableComponent<ReportingTask> newReportingTask = new LoggableComponent<>(newNode.getReportingTask(), newNode.getBundleCoordinate(),
                terminationAwareLogger);
